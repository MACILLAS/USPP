...
class MyNet(tf.keras.Model):
    def __init__(self, filters=100):
        ...
        self.relu1 = ReLU()
        self.bn1 = BatchNormalization(epsilon=0.00001, momentum=0.1)
        ...
        self.conv21 = Conv2D(filters=filters, kernel_size=3, strides=1, padding='same')
        self.relu21 = ReLU()
        self.bn21 = BatchNormalization(epsilon=0.00001, momentum=0.1)
        self.conv22 = Conv2D(filters=filters, kernel_size=3, strides=1, padding='same')
        self.relu22 = ReLU()
        self.bn22 = BatchNormalization(epsilon=0.00001, momentum=0.1)
        self.conv23 = Conv2D(filters=filters, kernel_size=3, strides=1, padding='same')
        self.relu23 = ReLU()
        self.bn23 = BatchNormalization(epsilon=0.00001, momentum=0.1)
    @tf.function
...
def segment(im, scribble=None, minLabels=3, nChannel=100, lr=0.01, stepsize_sim=1, stepsize_con=1, stepsize_scr=0.5, maxIter=500):
    """
...
def mainProgram():
    ...
    # build model instance
    row, col, chn = data.shape
    model = MyNet(filters=args.nChannel)
    model = model.build(row, col, chn)
